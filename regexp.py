...
What is needed? simple program which acts as a HTTP client . Program should
implement GET method and should be eought to to download files as one would
with the command line program curl It should include 1) Host: Header
2) Transfer- encoding : chunked
3) Connection: close header, or handle persistent connections
What is to be implemented? retrieve_url function which takes url(str) as its
 only argument
...
- may communicate within aprocess, between processes on the same machine or
